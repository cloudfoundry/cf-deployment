...
test_legacy_ops() {
  ...
  pushd ${home} > /dev/null
    pushd operations/legacy > /dev/null
      check_interpolation "name: keep-haproxy-ssl-pem.yml" "${home}/operations/use-haproxy.yml -o keep-haproxy-ssl-pem.yml -v haproxy_ssl_pem=i-am-a-pem -v haproxy_ssl_ca.certificate=i-am-a-ca-cert -v haproxy_private_ip=1.2.3.4"
