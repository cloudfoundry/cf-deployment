...
test_backup_and_restore_ops() {
  ...
  pushd ${home} > /dev/null
    pushd operations/backup-and-restore > /dev/null
      ...
      check_interpolation "name: enable-backup-restore-credhub-postgres.yml" "enable-backup-restore.yml" "-o ${home}/operations/experimental/secure-service-credentials.yml" "-o ${home}/operations/use-postgres.yml" "-o ${home}/operations/experimental/secure-service-credentials-postgres.yml" "-o enable-backup-restore-credhub-postgres.yml"
      check_interpolation "name: enable-backup-restore-credhub-external-db.yml" "enable-backup-restore.yml" "-o ${home}/operations/experimental/secure-service-credentials.yml" "-o ${home}/operations/experimental/secure-service-credentials-external-db.yml" "-o enable-backup-restore-credhub-external-db.yml"
      ...
      check_interpolation "name: enable-backup-restore-s3-unversioned.yml" "enable-backup-restore.yml" "-o ${home}/operations/use-external-blobstore.yml" "-o enable-backup-restore-s3-unversioned.yml" "-l ${home}/operations/example-vars-files/vars-use-s3-blobstore.yml" "-l  example-vars-files/vars-enable-backup-restore-s3-unversioned.yml"
