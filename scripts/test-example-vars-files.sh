        ...
        if [ "$(are_all_present_variables_used)" = "no" ]; then
          ...
          if [ "$(are_all_needed_variables_present)" = "yes" ]; then
            ...
            if [ "$(is_it_the_error_we_expect)" = "no" ]; then
              ...
              if [ "$(are_missing_variables_in_cf_d)" = "yes" ]; then
                pass "$ops_file"
              else
                fail "$ops_file failed because not all missing variables are in cf_d, namely: "$(are_missing_variables_in_cf_d)""
              fi
            fi
          fi
        fi
...
are_missing_variables_in_cf_d() {
  ...
  potentially_missing_variables="$(yq read /tmp/yaml-errors.txt)"
  are_variables_missing="false"
  missing_variables_list=""
  for variable in $potentially_missing_variables; do
    length="$(echo $cf_d_variables | jq --arg variable "$variable" 'map(select(. == $variable))' | jq length)"
    if [ $length != "1" ]; then
      are_variables_missing="true"
      missing_variables_list="${variable} ${missing_variables_list}"
    fi
  done
  if [ $are_variables_missing = "true" ]; then
    echo $missing_variables_list
  else
    echo "yes"
  fi
}
