...
| [`enable-service-discovery.yml`](enable-service-discovery.yml) | Enables application service discovery | Requires `use-bosh-dns-for-containers.yml`, and requires configuring at least one internal domain. Configure the default domain by running `cf curl /v2/shared_domains -d '{
 "name": "apps.internal",
 "internal": true
}`|
