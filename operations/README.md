...
| [`enable-nfs-ldap.yml`](enable-nfs-ldap.yml) | Enables LDAP authentication for NFS volume services | Introduces variables `nfs-ldap-service-user`, `nfs-ldap-service-password`, `nfs-ldap-host`, `nfs-ldap-port`, `nfs-ldap-proto`, and `nfs-ldap-fqdn`. |
