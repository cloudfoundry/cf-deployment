...
| [`use-trusted-ca-cert-for-apps.yml`](use-trusted-ca-cert-for-apps.yml) | Injects the CA specified with `trusted_cert_for_apps` into the Diego trusted store | This store determines which CAs will be trusted in the app environment. <br/> **CAUTION:** Replaces certs for both linux and rep, which have different `properties` formatting. Please take note. |
