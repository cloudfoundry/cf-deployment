...
| **GCP** | | |
| [`use-gcs-blobstore-service-account.yml`](use-gcs-blobstore-service-account.yml) | Enables service account credentials for Google blobstore. | Requires `use-external-blobstore.yml`. Introduces [new variables](example-vars-files/vars-use-gcs-blobstore-service-account.yml) for gcp service account credentials. |
| [`use-gcs-blobstore-access-key.yml`](use-gcs-blobstore-access-key.yml) | Enables access key credentials for Google blobstore. | Requires `use-external-blobstore.yml`. Introduces [new variables](example-vars-files/vars-use-gcs-blobstore-access-key.yml) for bucket names. Gcp creds are passed in. |
| **Openstack** | | |
